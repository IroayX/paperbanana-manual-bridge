...
def main():
    ...
    with tab1:
        ...
        # Example content
        example_method = r"""## Methodology: Chat-Bridge Manual Visualizer Pipeline

This project implements a practical variant of PaperBanana for environments where direct image-generation API calls are unstable or unavailable. The goal is to preserve the original multi-agent logic while replacing only the image generation step with a manual human-in-the-loop operation.

### Stage 1: Retriever
Given source context $S$ and visual intent $C$, the retriever selects top references from the benchmark pool. In auto mode, retrieval can be chunked and iterated until a final top-10 set is produced; in manual mode, the user may provide selected IDs directly.

### Stage 2: Planner
Planner consumes $(S, C)$ and top references to produce a detailed structural description of the target figure, including layout, labels, arrows, and semantic constraints.

### Stage 3: Stylist
Stylist refines Planner output using style guidelines to improve publication readiness while preserving semantics.

### Stage 4: Visualizer (Manual)
Instead of calling an image API internally, the system generates a strict visualizer prompt. The user runs this prompt in an external chat/agent model to create the image, then uploads the result back to the pipeline. Optional reference-assisted visualizer prompting supports two modes:
- **agent mode**: provide local reference image paths;
- **chat_only mode**: upload references in one or more batches and finish with `[ALL_REFERENCES_UPLOADED]`.

### Stage 5: Critic
Critic compares generated image against $(S, C)$ and current description, then outputs strict JSON:
`{"critic_suggestions":"...","revised_description":"..."}`.
If changes are needed, revised description is sent to the next visualizer round.

### Iterative Loop and Controls
The Visualizer-Critic loop runs for multiple rounds until convergence or `max_critic_rounds`. Text API Assist can automate text-only stages (retriever/stylist/critic) with configurable base URL, model, and token settings. Visualizer remains manual by design.

### Output Artifacts
The system stores prompts, stage outputs, round traces, and uploaded images, then reports a recommended final image (latest valid visualizer result)."""

        example_caption = "Figure 1: Overview of the Chat-Bridge manual visualizer workflow. Retriever, Planner, and Stylist generate structured textual guidance; image generation is executed manually outside the pipeline at the Visualizer step; Critic returns JSON feedback for iterative refinement. The process preserves PaperBanana stage order while enabling robust operation without internal image API calls."
        ...
        with col_input1:
            ...
            method_example = st.selectbox(
                "Load Example (Method)",
                ["None", "Chat-Bridge Manual Visualizer"],
                key="method_example_selector"
            )
            
            # Set value based on example selection or session state
            if method_example == "Chat-Bridge Manual Visualizer":
        ...
        with col_input2:
            ...
            # Set value based on example selection or session state
            if caption_example == "Chat-Bridge Manual Visualizer":
